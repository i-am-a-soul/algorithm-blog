export const getMathematicsSidebar = () => [{
  text: '质数',
  children: [
    '/solution/04.mathematics/4.1 质数/题解-质数距离-质数.md',
    '/solution/04.mathematics/4.1 质数/题解-阶乘分解-分解质因数.md',
  ],
}, {
  text: '约数',
  children: [
    '/solution/04.mathematics/4.2 约数/题解-约数研究-约数.md',
    '/solution/04.mathematics/4.2 约数/题解-轻拍牛头-约数.md',
    '/solution/04.mathematics/4.2 约数/题解-余数之和-整除分块.md',
    '/solution/04.mathematics/4.2 约数/题解-Hankson的趣味题-最大公约数.md',
    '/solution/04.mathematics/4.2 约数/题解-仪仗队-欧拉函数.md',
    '/solution/04.mathematics/4.2 约数/题解-最大公约数-欧拉函数.md',
    '/solution/04.mathematics/4.2 约数/题解-Calculation 2-欧拉函数.md',
    '/solution/04.mathematics/4.2 约数/题解-反素数-DFS.md',
  ],
}, {
  text: '同余',
  children: [
    '/solution/04.mathematics/4.3 同余/题解-最幸运的数字-欧拉定理.md',
    '/solution/04.mathematics/4.3 同余/题解-约数之和-乘法逆元.md',
    '/solution/04.mathematics/4.3 同余/题解-青蛙的约会-线性同余方程.md',
  ],
}, {
  text: '矩阵乘法',
  children: [
    '/solution/04.mathematics/4.4 矩阵乘法/题解-斐波那契-矩阵快速幂.md',
    '/solution/04.mathematics/4.4 矩阵乘法/题解-Fibonacci 前 n 项和-矩阵快速幂.md',
    '/solution/04.mathematics/4.4 矩阵乘法/题解-佳佳的 Fibonacci-矩阵快速幂.md',
    '/solution/04.mathematics/4.4 矩阵乘法/题解-石头游戏-矩阵快速幂.md',
  ],
}, {
  text: '高斯消元与线性空间',
  children: [
    '/solution/04.mathematics/4.5 高斯消元与线性空间/题解-球形空间产生器-高斯消元.md',
    '/solution/04.mathematics/4.5 高斯消元与线性空间/题解-开关问题-异或线性方程组.md',
    '/solution/04.mathematics/4.5 高斯消元与线性空间/题解-装备购买-线性空间.md',
    '/solution/04.mathematics/4.5 高斯消元与线性空间/题解-xor序列-线性基.md',
    '/solution/04.mathematics/4.5 高斯消元与线性空间/题解-异或运算-线性基.md',
  ],
}, {
  text: '组合计数',
  children: [
    '/solution/04.mathematics/4.6 组合计数/题解-牡牛和牝牛-组合计数.md',
    '/solution/04.mathematics/4.6 组合计数/题解-车的放置-组合计数.md',
    '/solution/04.mathematics/4.6 组合计数/题解-数三角形-组合计数.md',
    '/solution/04.mathematics/4.6 组合计数/题解-序列统计-组合计数.md',
    '/solution/04.mathematics/4.6 组合计数/题解-古代猪文-扩展Lucas.md',
  ],
}, {
  text: '容斥原理与莫比乌斯函数',
  children: [
    '/solution/04.mathematics/4.7 容斥原理与莫比乌斯函数/题解-破译密码-莫比乌斯函数.md',
  ],
}, {
  text: '概率与数学期望',
  children: [
    '/solution/04.mathematics/4.8 概率与数学期望/题解-Rainbow的信号-概率.md',
    '/solution/04.mathematics/4.8 概率与数学期望/题解-绿豆蛙的归宿-概率.md',
    '/solution/04.mathematics/4.8 概率与数学期望/题解-扑克牌-概率.md',
  ],
}, {
  text: '01分数规划',
  children: [
  ],
}, {
  text: '博弈论',
  children: [
    '/solution/04.mathematics/4.10 博弈论/题解-Brave Game-巴什博弈.md',
    '/solution/04.mathematics/4.10 博弈论/题解-剪纸游戏-SG函数.md',
    '/solution/04.mathematics/4.10 博弈论/题解-台阶-Nim游戏-Nim游戏.md',
    '/solution/04.mathematics/4.10 博弈论/题解-集合-Nim游戏-SG函数.md',
    '/solution/04.mathematics/4.10 博弈论/题解-拆分-Nim游戏-SG函数.md',
    '/solution/04.mathematics/4.10 博弈论/题解-Anti-Nim游戏-Nim游戏.md',
  ],
}, {
  text: '计算几何',
  children: [
    '/solution/04.mathematics/4.11 计算几何/题解-平面最近点对-计算几何.md',
    '/solution/04.mathematics/4.11 计算几何/题解-斜率最大-计算几何.md',
  ],
}, {
  text: '其他',
  children: [
    '/solution/04.mathematics/4.12 其他/题解-H数.md',
    '/solution/04.mathematics/4.12 其他/题解-大大数星星.md',
    '/solution/04.mathematics/4.12 其他/题解-折线分割平面.md',
  ],
}];
